...
type CombinedStreamsClient struct {
	...
	batchSize   int // Number of streams to subscribe per batch
}
...
func (c *CombinedStreamsClient) Connect() error {
	...
	// Combined streams use different endpoints
	...
	log.Println("Combined stream WebSocket connection successful")
	go c.readMessages()

	return nil
}

// BatchSubscribeKlines batch subscribes to K-lines
func (c *CombinedStreamsClient) BatchSubscribeKlines(symbols []string, interval string) error {
	...
	for i, batch := range batches {
		log.Printf("Subscribing to batch %d, count: %d", i+1, len(batch))

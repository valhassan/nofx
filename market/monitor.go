...
type WSMonitor struct {
	...
	batchSize      int
	filterSymbols  sync.Map // Use sync.Map to store symbols to monitor and their status
...
type SymbolStats struct {
	...
	Score            float64 // Comprehensive score
}

var WSMonitorCli *WSMonitor
var subKlineTime = []string{"3m", "4h"} // Manage K-line intervals for subscribed streams
...
func (m *WSMonitor) Initialize(coins []string) error {
	...
	// If no trading pairs are specified, use all trading pairs from the market
	if len(coins) == 0 {
		exchangeInfo, err := apiClient.GetExchangeInfo()
		if err != nil {
			return err
		}
		// Filter perpetual contract trading pairs -- for testing only
...
func (m *WSMonitor) initializeHistoricalData() error {
	...
	for _, symbol := range m.symbols {
		...
		go func(s string) {
			...
			if err != nil {
				log.Printf("Failed to get historical data for %s: %v", s, err)
				return
			}
			if len(klines) > 0 {
				m.klineDataMap3m.Store(s, klines)
				log.Printf("Loaded %d 3m historical K-line data for %s", len(klines), s)
			}
			// Get historical K-line data
			...
			if len(klines4h) > 0 {
				...
				log.Printf("Loaded %d 4h historical K-line data for %s", len(klines4h), s)
			}
		}(symbol)
	}
...
func (m *WSMonitor) Start(coins []string) {
	...
	if err != nil {
		log.Printf("❌ Failed to initialize symbols: %v", err)
		return
	}

	err = m.combinedClient.Connect()
	if err != nil {
		log.Printf("❌ Failed to batch subscribe streams: %v", err)
		return
	}
	// Subscribe to all trading pairs
	err = m.subscribeAll()
	if err != nil {
		log.Printf("❌ Failed to subscribe to symbol trading pairs: %v", err)
		return
	}
}

// subscribeSymbol registers a listener
...
func (m *WSMonitor) subscribeAll() error {
	...
	log.Println("All trading pairs subscribed")
	return nil
}
...
func (m *WSMonitor) GetCurrentKlines(symbol string, _time string) ([]Kline, error) {
	// For each incoming symbol, check if it exists internally, and if so, subscribe to it
	value, exists := m.getKlineDataMap(_time).Load(symbol)
	if !exists {
		// If WebSocket data is not initialized, use API separately - compatibility code (to prevent traders from running before initialization is complete)
		apiClient := NewAPIClient()
		klines, err := apiClient.GetKlines(symbol, _time, 100)
		if err != nil {
			return nil, fmt.Errorf("Failed to get %v minute K-line: %v", _time, err)
		}
		...
		log.Printf("Dynamically subscribing to stream: %v", subStr)
		if subErr != nil {
			log.Printf("Warning: Failed to dynamically subscribe to %v minute K-line: %v (using API data)", _time, subErr)
		}
	...
	// ✅ FIX: Return deep copy instead of reference to avoid concurrent race conditions
	klines := value.([]Kline)
	result := make([]Kline, len(klines))
	copy(result, klines)
	return result, nil

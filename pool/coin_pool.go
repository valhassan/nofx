...
var coinPoolConfig = CoinPoolConfig{
	...
	UseDefaultCoins: false, // default not to use
}
...
func GetCoinPool() ([]CoinInfo, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		if attempt > 1 {
			log.Printf("⚠️  Retry %d/%d to get coin pool...", attempt, maxRetries)
...
// GetAvailableCoins gets available coin list (filtering unavailable ones)
...
func GetOITopPositions() ([]OIPosition, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		if attempt > 1 {
			log.Printf("⚠️  Retry %d/%d to get OI Top data...", attempt, maxRetries)
		...
		lastErr = err
		log.Printf("❌ Request %d for OI Top failed: %v", attempt, err)
	}
